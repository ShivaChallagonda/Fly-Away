			...
			try {
				...
				for (user p : obj) {
					if (p.getPassword().equals(password)) {
						...
						for (flight x : obj2) {
							if (x.getId() == val) {
								temp = (double) x.getPrice() * price; // if user has booked tickets for 2 people then
																		// 2*ticketprice
							}
						}
						int userid = p.getId();
						tempf = p.getBalance() - temp;
						session.createSQLQuery("UPDATE user_data" + "SET balance=" + tempf + "WHERE id=" + userid)
								.executeUpdate();
						...
						if (tempf < 0) {
							...
							Query<Update> q = session.createQuery("update user set balance=:n where id=:i");
